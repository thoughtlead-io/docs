...
Sending authenticated requests to the Thoughtly API requires two headers present in every request. These values can be found in the [Thoughtly dashboard](https://app.thoughtly.com/settings/developers).

1. `x-api-key`: Your API key.
2. `team_id`: To support multi-tenancy, you must include the `team_id` of the team you want to access.
